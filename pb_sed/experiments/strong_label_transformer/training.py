...
ex_name = 'strong_label_transformer_training'
ex = Exp(ex_name)
...
def config():
    ...
    database_name = 'desed'
    storage_dir = str(storage_root / 'strong_label_transformer' / database_name / 'training' / group_name / timestamp)
    resume = False
    if resume:
    ...
    trainer = {
        'model': {
            'factory': strong_label.Transformer,
            'feature_extractor': {
                'sample_rate':
...
def train(
    ...
    if validation_set_name:
        tuning.run(
            config_updates={
                'debug': debug,
                'weak_label_transformer_hyper_params_dir': weak_label_transformer_hyper_params_dir,
                'strong_label_transformer_dirs': [str(trainer.storage_dir)],
