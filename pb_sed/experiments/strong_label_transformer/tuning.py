...
from pb_sed.experiments.weak_label_transformer.inference import tagging
from pb_sed.experiments.strong_label_transformer.inference import ex as evaluation


ex_name = 'strong_label_transformer_hyper_params'
ex = Exp(ex_name)
...
def config():
    ...
    weak_label_transformer_hyper_params_dir = ''
    ...
    strong_label_transformer_group_dir = ''
    if isinstance(strong_label_transformer_group_dir, list):
        strong_label_transformer_dirs = sorted([
            str(d) for g in strong_label_transformer_group_dir for d in Path(g).glob('202*') if d.is_dir()
        ])
    else:
        strong_label_transformer_dirs = sorted([str(d) for d in Path(strong_label_transformer_group_dir).glob('202*') if d.is_dir()])
    assert len(strong_label_transformer_dirs) > 0, 'strong_label_transformer_dirs must not be empty.'
    strong_label_transformer_checkpoints = 'ckpt_best_macro_fscore_strong.pth'
    strong_transformer_config = load_json(Path(strong_label_transformer_dirs[0]) / '1' / 'config.json')
    data_provider = strong_transformer_config['data_provider']
    database_name = strong_transformer_config.get('database_name', 'desed')
    storage_dir = str(storage_root / 'strong_label_transformer' / database_name / 'hyper_params' / timestamp)
    assert not Path(storage_dir).exists()
    del strong_transformer_config
...
def main(
    ...
    strong_label_transformers = [
        ...
        for transformer_dir, transformer_checkpoint in zip(
            strong_label_transformer_dirs, strong_label_transformer_checkpoints)
    ]
    ...
    leaderboard = strong_label.transformer.tune_sound_event_detection(
        strong_label_transformers, dataset.map(add_tag_condition), device, timestamps,
    ...
    for transformer_dir in strong_label_transformer_dirs:
        tuning_dir = Path(transformer_dir) / 'hyper_params'
    ...
    if eval_set_name:
        evaluation.run(
            config_updates={
                'debug': debug,
                'strong_label_transformer_hyper_params_dir': str(storage_dir),
