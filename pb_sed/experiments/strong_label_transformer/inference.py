...
ex_name = 'strong_label_transformer_inference'
ex = Exp(ex_name)
...
def config():
    ...
    strong_label_transformer_hyper_params_dir = ''
    assert len(strong_label_transformer_hyper_params_dir) > 0, 'Set strong_label_transformer_hyper_params_dir on the command line.'
    strong_label_transformer_tuning_config = load_json(Path(strong_label_transformer_hyper_params_dir) / '1' / 'config.json')
    strong_label_transformer_dirs = strong_label_transformer_tuning_config['strong_label_transformer_dirs']
    assert len(strong_label_transformer_dirs) > 0, 'strong_label_transformer_dirs must not be empty.'
    strong_label_transformer_checkpoints = strong_label_transformer_tuning_config['strong_label_transformer_checkpoints']
    data_provider = strong_label_transformer_tuning_config['data_provider']
    database_name = strong_label_transformer_tuning_config['database_name']
    storage_dir = str(storage_root / 'strong_label_transformer' / database_name / 'inference' / timestamp)
    assert not Path(storage_dir).exists()

    weak_label_transformer_hyper_params_dir = strong_label_transformer_tuning_config['weak_label_transformer_hyper_params_dir']
    assert len(weak_label_transformer_hyper_params_dir) > 0, 'Set weak_label_transformer_hyper_params_dir on the command line.'
    weak_label_transformer_tuning_config = load_json(Path(weak_label_transformer_hyper_params_dir) / '1' / 'config.json')
    weak_label_transformer_dirs = weak_label_transformer_tuning_config['transformer_dirs']
    assert len(weak_label_transformer_dirs) > 0, 'weak_label_transformer_dirs must not be empty.'
    weak_label_transformer_checkpoints = weak_label_transformer_tuning_config['transformer_checkpoints']

    del strong_label_transformer_tuning_config
    del weak_label_transformer_tuning_config
...
def sound_event_detection(
        transformers, dataset, device, timestamps, event_classes, tags,
...
def main(
        _run,
        storage_dir, strong_label_transformer_hyper_params_dir, sed_hyper_params_name,
        strong_label_transformer_dirs, strong_label_transformer_checkpoints,
        weak_label_transformer_hyper_params_dir, weak_label_transformer_dirs, weak_label_transformer_checkpoints,
    ...
    if not isinstance(weak_label_transformer_checkpoints, list):
        assert isinstance(weak_label_transformer_checkpoints, str), weak_label_transformer_checkpoints
        weak_label_transformer_checkpoints = len(weak_label_transformer_dirs) * [weak_label_transformer_checkpoints]
    weak_label_transformers = [
        weak_label.transformer.from_storage_dir(
            storage_dir=transformer_dir, config_name='1/config.json',
            checkpoint_name=transformer_checkpoint
        )
        for transformer_dir, transformer_checkpoint in zip(weak_label_transformer_dirs, weak_label_transformer_checkpoints)
    ]
    print('Weak Label transformer Params', sum([p.numel() for transformer in weak_label_transformers for p in transformer.parameters()]))
    print('Weak Label CNN2d Params', sum([p.numel() for transformer in weak_label_transformers for p in transformer.cnn.cnn_2d.parameters()]))
    if not isinstance(strong_label_transformer_checkpoints, list):
        assert isinstance(strong_label_transformer_checkpoints, str), strong_label_transformer_checkpoints
        strong_label_transformer_checkpoints = len(strong_label_transformer_dirs) * [strong_label_transformer_checkpoints]
    strong_label_transformers = [
        strong_label.transformer.from_storage_dir(
            storage_dir=transformer_dir, config_name='1/config.json',
            checkpoint_name=transformer_checkpoint
        )
        for transformer_dir, transformer_checkpoint in zip(strong_label_transformer_dirs, strong_label_transformer_checkpoints)
    ]
    print('Strong Label transformer Params', sum([p.numel() for transformer in strong_label_transformers for p in transformer.parameters()]))
    print('Strong Label CNN2d Params', sum([p.numel() for transformer in strong_label_transformers for p in transformer.cnn.cnn_2d.parameters()]))
    ...
    for i in range(len(dataset_name)):
        ...
        tags, tagging_scores, _ = tagging(
            weak_label_transformers, dataset, device, timestamps, event_classes,
            weak_label_transformer_hyper_params_dir, None, None,
        )
        ...
        events, sed_results = sound_event_detection(
            strong_label_transformers, dataset, device, timestamps, event_classes, tags,
            strong_label_transformer_hyper_params_dir, sed_hyper_params_name,
    ...
    inference_dir = Path(strong_label_transformer_hyper_params_dir) / 'inference'
